...
/**
 * Detect a mac address The main logic is: 0) set the filter 1) compose the
 * packet 2) start the receive thread task 3) send tha packet 4) wait response
 * task result or timeout happen
 * <p>
...
public class MacAddressHelper {
    ...
    /**
     * NOTICE, the pcap will block for those time even when the data
     * returned....
     */
    private int _readTimeoutInMillSeconds = 100; // 100 ms for reading a
                                                 // response from the packet
    private int _waitResponseTimeoutInMillSeconds = 2000; // 2 second, this
                                                          // should be quick
                                                          // enough under same
                                                          // switch
    ...
    static {
        // for linux, only required libpcap, so you see the filename is with
        // version
        // for windows, we need Packet.dll and wpcap.dll, and wpcap required
        // Packet.dll so the filename is WITHOUT version
        String pcapLibKey = "org.pcap4j.core.pcapLibName";
        String packetDllKey = "org.pcap4j.core.packetLibName";
        // if (Environment.isLinux()) {
        // System.setProperty(pcapLibKey, Paths.get(".", "lib",
        // Environment.is64BitOS() ? "libpcap64.so" : "libpcap.so")
        // .toString());
        // }
        // else {
        // System.setProperty(packetDllKey, Paths.get(Environment.root, "lib",
        // "Packet.dll").toString());
        // System.setProperty(pcapLibKey, Paths.get(Environment.root, "lib",
        // "wpcap.dll").toString());
        // }
        // System.out.println(String.format("Pcap related conf set %s=%s, (for windows) %s=%s",
        // pcapLibKey, System.getProperty(pcapLibKey),
        ...
        try {
            ...
            while (localNetworkInterfaces.hasMoreElements()) {
                ...
                while (addresses.hasMoreElements()) {
                    ...
                    if (mac == null) {
                        ...
                        // although jdk said, the returned value is a mac
                        // address, but actually this may fail on some local mac
                        // address
                        // the length is NOT 6 bytes
                        if (mac.length != MacAddress.SIZE_IN_BYTES) {
                            _localAddresse2MacAddress.put(currentIp, null);
                            System.out.println(String.format("Found invalid mac address ip=%s,mac=%s", currentIp,
                                    ByteArrays.toHexString(mac, ":")));
                        } else {
            ...
            System.out
                    .println(String.format(
                            "Mac Address helper init done localips=%d, threadPool=%d, readTimeout(ms)=%d, waitResponse(ms)"
                                    + "=%d, waitReceiveTaskStart(ms)=%d", _localAddresse2MacAddress.size(),
                            _threadCount, _readTimeoutInMillSeconds, _waitResponseTimeoutInMillSeconds,
                            _waitReceiveTaskStartRunningInSeconds));
            _initted = true;
        } catch (Throwable e) {
            _initError = e;
        }
     ...
     */
    private MacAddress _getMacAddress(InetAddress inetAddress) throws PcapNativeException, NotOpenException,
        ...
        MacAddress localMacAddress = MacAddress.getByAddress(intf._selectedNetworkInterface.getLinkLayerAddresses()
                .get(0).getAddress());
        ...
        try {
            ...
            receiveHandle = intf._selectedNetworkInterface.openLive(_snapLen,
                    PcapNetworkInterface.PromiscuousMode.PROMISCUOUS, _readTimeoutInMillSeconds);
            ...
            sendHandle = intf._selectedNetworkInterface.openLive(_snapLen,
                    PcapNetworkInterface.PromiscuousMode.PROMISCUOUS, _readTimeoutInMillSeconds);
            ...
            EthernetPacket.Builder etherBuilder = isIPv4Address ? _getPacketBuilder4IPv4(inetAddress, localMacAddress,
                    localIpAddress) : _getPacketBuilder4IPv6((Inet6Address) inetAddress, localMacAddress,
                    (Inet6Address) localIpAddress);
    ...
    private String _getFilter4IPv4(InetAddress remoteAddress, MacAddress localMacAddress, InetAddress localIpAdress) {
        return String.format("arp and src host %s and dst host %s and ether dst %s", remoteAddress.getHostAddress(),
                localIpAdress.getHostAddress(), Pcaps.toBpfString(localMacAddress));
    }
    ...
    private String _getFilter4IPv6(Inet6Address remoteAddress, MacAddress localMacAddress, Inet6Address localIpAdress) {
        return String.format("icmp6 and src host %s and dst host %s and ether dst %s",
                ...
                _removePercentInIPv6Address(localIpAdress.getHostAddress()), Pcaps.toBpfString(localMacAddress));
    }

    private EthernetPacket.Builder _getPacketBuilder4IPv4(InetAddress remoteAddress, MacAddress localMacAddress,
            InetAddress localIpAdress) {
        ArpPacket.Builder arpBuilder = new ArpPacket.Builder();
        arpBuilder.hardwareType(ArpHardwareType.ETHERNET).protocolType(EtherType.IPV4)
                .hardwareAddrLength((byte) MacAddress.SIZE_IN_BYTES)
                .protocolAddrLength((byte) ByteArrays.INET4_ADDRESS_SIZE_IN_BYTES).srcHardwareAddr(localMacAddress)
                .srcProtocolAddr(localIpAdress).dstHardwareAddr(MacAddress.ETHER_BROADCAST_ADDRESS)
                .operation(ArpOperation.REQUEST).dstProtocolAddr(remoteAddress);

        EthernetPacket.Builder etherBuilder = new EthernetPacket.Builder();
        etherBuilder.dstAddr(MacAddress.ETHER_BROADCAST_ADDRESS).srcAddr(localMacAddress).type(EtherType.ARP)
                .payloadBuilder(arpBuilder).paddingAtBuild(true);

        return etherBuilder;
    }

    private EthernetPacket.Builder _getPacketBuilder4IPv6(Inet6Address remoteAddress, MacAddress localMacAddress,
            Inet6Address localIpAdress) throws UnknownHostException {
        ...
        IpV6NeighborDiscoverySourceLinkLayerAddressOption.Builder optionBuilder = new IpV6NeighborDiscoverySourceLinkLayerAddressOption.Builder();
        IpV6NeighborDiscoverySourceLinkLayerAddressOption option = optionBuilder
                .linkLayerAddress(localMacAddress.getAddress()).correctLengthAtBuild(true).build();
        ...
        icmpV6b.type(IcmpV6Type.NEIGHBOR_SOLICITATION).code(IcmpV6Code.NO_CODE).srcAddr(localIpAdress)
                .dstAddr(broadcasetIPAddress).payloadBuilder(v6Builder).correctChecksumAtBuild(true);

        IpV6Packet.Builder ipv6b = new IpV6Packet.Builder();
        ipv6b.version(IpVersion.IPV6).trafficClass(IpV6SimpleTrafficClass.newInstance((byte) 0x12))
                .flowLabel(IpV6SimpleFlowLabel.newInstance(0)).nextHeader(IpNumber.ICMPV6).hopLimit((byte) 255)
                .srcAddr(localIpAdress).dstAddr(broadcasetIPAddress) // "fe80:0:0:0:250:56ff:febc:2688"
                                                                     // ->
                                                                     // "FF02::1:FFbc:2688"
                .correctLengthAtBuild(true).payloadBuilder(icmpV6b);

        EthernetPacket.Builder etherBuilder = new EthernetPacket.Builder();
        etherBuilder.dstAddr(broadcastMacAddress).srcAddr(localMacAddress).type(EtherType.IPV6).payloadBuilder(ipv6b)
                .paddingAtBuild(true);
        ...
        for (PcapNetworkInterface currentInterface : _localPcapNetworkInterfaces) {
            ...
            if (addresses != null) {
                for (PcapAddress ipAddress : addresses) {
                    // make sure the address should be same type, all ipv4 or
                    // all ipv6
    ...
    /**
     * The broadcast ip for a icmp v6 request is replaced all bytes except the
     * last three bytes with _IPv6_BROADCAST_IPADDRESS_PREFIX
     *
     * @param inet6Address
     * @return
     */
    private Inet6Address _getBroadcastIPAddress4IPv6(Inet6Address inet6Address) throws UnknownHostException {
        ...
        System.arraycopy(_IPv6_BROADCAST_IPADDRESS_PREFIX, 0, broadcastIpAddress, 0,
                _IPv6_BROADCAST_IPADDRESS_PREFIX.length);
        int reservedBytes = 3;
        System.arraycopy(ipInBytes, ipInBytes.length - reservedBytes, broadcastIpAddress,
                _IPv6_BROADCAST_IPADDRESS_PREFIX.length - reservedBytes, reservedBytes);
        return (Inet6Address) Inet6Address.getByAddress(broadcastIpAddress);
    }

    /**
     * The broadcast mac address for a icmpv6 request is replaced all bytes with
     * 33:33:ff except the last thress bytes fe80::250:56ff:fe95:f8d ->
     * 33:33:ff:95:0f:8d
     *
     * @param inet6Address
     * @return
    ...
    private MacAddress _getBroadcastMacAddress4IPv6(Inet6Address inet6Address) {
        ...
        System.arraycopy(_IPv6_BROADCAST_MACADDRESS_PREFIX, 0, broadcastMacAddress, 0,
                _IPv6_BROADCAST_MACADDRESS_PREFIX.length);
        int reservedBytes = 3;
        System.arraycopy(ipInBytes, ipInBytes.length - reservedBytes, broadcastMacAddress,
                _IPv6_BROADCAST_MACADDRESS_PREFIX.length - reservedBytes, reservedBytes);
    ...
    private static class MacAddressHelperHolder {
        ...
        // and the static block in the MacAddressHelper will run before this
        // construction
    ...
    /**
     * @param args
     *            the remote device ip lists split by comma
     */
