...
/**
 * dump packets Copy from pcap4j examples The filter syntax is -
 * https://biot.com/capstats/bpf.html
 * <p>
 * When it's working for dns, you may got an IllegalArgumentException, Which
 * fixed in here: https://github.com/kaitoy/pcap4j/issues/123
 */
public class PacketDump {
    ...
    private static final boolean QUIET = Boolean.getBoolean(QUIET_KEY); // default
                                                                        // is
                                                                        // false
    ...
    private static final String PCAP_FILE = System
            .getProperty(PCAP_FILE_KEY, "Dump" + sdf.format(new Date()) + ".pcap");
    ...
    static void usage() {
        ...
        System.out.println(String.format("\t%-20s -> %s", COUNT_KEY,
                "Packets numbers. Not supported Anymore. use -Drun. "));
        ...
        System.out.println(String.format("\t%-20s -> %s", QUIET_KEY,
                "Run in quiet mode? if it's true, you should set the - " + INTF_KEY));
    ...
    private static PcapNetworkInterface _selectNif() throws Exception {
        ...
        int nifIdx = 0;
        while (true) {
            write(String.format("Select a device number ( 0 - %d) to capture packets, or enter 'q' to quit > ",
                    allDevs.size() - 1));
            String input;
            if ((input = read()) == null) {
                continue;
